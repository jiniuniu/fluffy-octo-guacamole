...
SVG_MODIFY_PROMPT_TEMPLATE = """你需要根据用户反馈改进现有的SVG物理动画。
...
## 修改指导原则
1. **保持物理准确性** - 不要改变基本的物理原理和解释逻辑
2. **精准响应反馈** - 直接解决用户提到的具体问题
3. **保持动画结构** - 保持现有的动画类型和基本布局
4. **优化用户体验** - 让动画更清晰、易懂

## 常见修改类型及方法
- **速度调整**: 修改 `dur` 值 (慢一点用更大的数值，快一点用更小的数值)
- **颜色优化**: 改善 `fill`、`stroke` 颜色，提升对比度
- **文字问题**: 调整 `<text>` 的位置、大小、颜色，避免重叠
- **元素大小**: 调整圆形的 `r`、矩形的 `width/height` 等
- **动画效果**: 优化 `animate` 和 `animateTransform` 的参数
- **标签完善**: 添加缺失的说明文字或图例
...
## 重要提醒
⚠️ 文本格式要求：
- SVG代码中不要使用双引号（"），统一使用单引号（'）
- 避免反斜杠（\）和其他可能导致JSON解析错误的特殊字符

基于用户的具体反馈，请输出改进后的完整SVG代码。
