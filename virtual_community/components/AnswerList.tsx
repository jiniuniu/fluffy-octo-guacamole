...
export function AnswerList({
  ...
  const feed = useMemo(() => {
    ...
    const explorer =
      rest.length > 0 ? rest[Math.floor(Math.random() * rest.length)] : null;
    ...
    <div className="divide-y divide-border">
      {feed.map((answer) => {
        ...
        return (
          <div
            key={answer._id}
            className={`mx-auto max-w-3xl px-4 py-3 ${STANCE_BORDER[answer.stance]}`}
          >
            {/* explorer badge */}
            {isExplorer && (
            ...
            <div className="mt-1.5 flex items-center gap-3">
              ...
              {!readonly && <ReplyInput answerId={answer._id} inline />}
            </div>
