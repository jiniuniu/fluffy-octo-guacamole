...
export function ActivityLog({ questionId }: { questionId: Id<"questions"> }) {
  const logs = useQuery(api.activity_log.byQuestion, {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
        {!logs && <p className="text-xs text-muted-foreground">等待中...</p>}
        {logs?.map((log) => {
          ...
          return (
            <div key={log._id} className="flex items-start gap-1.5 text-xs">
              {persona ? (
                ...
                <span className="shrink-0 rounded bg-muted px-1 py-0.5 font-medium text-foreground">
                  ...
                </span>
              )}
